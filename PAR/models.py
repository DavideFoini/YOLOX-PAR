...
class MultiTask(torch.nn.Module):
    def __init__(self, load=False, path=None, num_classes=25):
        ...
        self.gender = nn.Sequential(nn.Linear(in_features=256, out_features=512),
                                    nn.ReLU(),
                                    nn.Linear(in_features=512, out_features=1))

        self.label = nn.Linear(in_features=256, out_features=num_classes)
...
class MultiTask2(torch.nn.Module):
    def __init__(self, load=False, path=None, num_classes=25):
        ...
        self.name = "MultiTaskV2"

    def forward(self, x):
        back_out = self.backbone(x)[self.backbone.out_features[0]]
        label_in = self.avg_pool(back_out)
        label_in = label_in.squeeze(axis=(2,3))
        label_out = self.label(label_in)
...
class MultiTask3(torch.nn.Module):
    def __init__(self, load=False, path=None, num_classes=22):
    ...
    def forward(self, x):
        ...
        return gender_out, pov_out, label_out
...
class MultiTask4(torch.nn.Module):
